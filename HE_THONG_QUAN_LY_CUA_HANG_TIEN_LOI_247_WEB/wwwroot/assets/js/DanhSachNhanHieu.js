document.addEventListener('DOMContentLoaded', function () {
    console.log('=== DANH SÁCH NHÃN HIỆU PAGE LOADED ===');

    // --- TRUY XUẤT PHẦN TỬ DOM ---
    const mainRow = document.getElementById('main-row-container');
    const listCol = document.getElementById('list-col');
    const formCol = document.getElementById('form-col');
    const showAddFormBtn = document.getElementById('show-add-form-btn');

    const nhanHieuForm = document.getElementById('nhan-hieu-form');
    const formTitle = document.getElementById('form-title');

    // Input fields
    const inputId = document.getElementById('form-nhan-hieu-id');
    const inputTen = document.getElementById('form-nhan-hieu-ten');

    // Buttons
    const btnSubmit = document.getElementById('btn-submit');
    const btnCancel = document.getElementById('btn-cancel');
    const btnEditMode = document.getElementById('btn-edit-mode');

    let currentMode = null; // 'add', 'edit', 'view'
    let currentData = null;

    // --- HÀM MỞ FORM ---
    function openForm(mode, data = null) {
        console.log('openForm:', mode, data);
        currentMode = mode;
        currentData = data;

        // Co bảng, mở form
        listCol.classList.remove('col-md-12');
        listCol.classList.add('col-md-8');
        formCol.classList.remove('col-md-0');
        formCol.classList.add('col-md-4');
        mainRow.classList.add('form-open');
        showAddFormBtn.style.display = 'none';

        nhanHieuForm.classList.remove('form-readonly');

        if (mode === 'add') {
            formTitle.innerText = 'Thêm Nhãn Hiệu Mới';
            nhanHieuForm.reset();

            // Lấy ID tự động
            callApiGetNextIdNH();

            inputId.readOnly = true;
            btnSubmit.innerHTML = '<i class="bi bi-check-circle-fill me-2"></i>Thêm';
            btnSubmit.style.display = 'inline-block';
            btnCancel.innerHTML = '<i class="bi bi-x-circle-fill me-2"></i>Huỷ';
            btnCancel.style.display = 'inline-block';
            btnEditMode.style.display = 'none';
        }
        else if (mode === 'edit' || mode === 'view') {
            // Điền dữ liệu
            inputId.value = data.id;
            inputTen.value = data.ten;

            if (mode === 'view') {
                formTitle.innerText = 'Chi Tiết Nhãn Hiệu';
                nhanHieuForm.classList.add('form-readonly');
                btnSubmit.style.display = 'none';
                btnCancel.innerText = 'Đóng';
                btnCancel.style.display = 'inline-block';
                btnEditMode.style.display = 'inline-block';
            } else {
                formTitle.innerText = 'Sửa Nhãn Hiệu';
                inputId.readOnly = true;
                btnSubmit.innerHTML = '<i class="bi bi-check-circle-fill me-2"></i>Lưu';
                btnSubmit.style.display = 'inline-block';
                btnCancel.innerText = 'Huỷ';
                btnCancel.style.display = 'inline-block';
                btnEditMode.style.display = 'none';
            }
        }
    }

    // --- HÀM ĐÓNG FORM ---
    function closeForm() {
        listCol.classList.remove('col-md-8');
        listCol.classList.add('col-md-12');
        formCol.classList.remove('col-md-4');
        formCol.classList.add('col-md-0');
        mainRow.classList.remove('form-open');
        showAddFormBtn.style.display = 'block';

        nhanHieuForm.classList.remove('form-readonly');
        nhanHieuForm.reset();
        currentData = null;
        currentMode = null;
    }

    // --- GÁN SỰ KIỆN ---
    showAddFormBtn.addEventListener('click', function (e) {
        e.preventDefault();
        openForm('add');
    });

    btnCancel.addEventListener('click', function (e) {
        e.preventDefault();
        closeForm();
    });

    btnEditMode.addEventListener('click', function () {
        if (currentData) {
            openForm('edit', currentData);
        }
    });

    btnSubmit.addEventListener('click', async function (e) {
        e.preventDefault();

        if (currentMode === 'add') {
            await callApiAddNH();
        } else if (currentMode === 'edit') {
            await callApiEditNH();
        }
    });

    // Xử lý click vào bảng
    document.querySelector('#sampleTable tbody').addEventListener('click', function (e) {
        const clickedRow = e.target.closest('tr');
        if (!clickedRow) return;

        // Lấy data
        const data = {
            id: clickedRow.dataset.id,
            ten: clickedRow.dataset.ten
        };

        // Xử lý nút XÓA
        if (e.target.closest('.btn-delete-khoi')) {
            e.stopPropagation();

            if (confirm(`Bạn có chắc muốn xoá nhãn hiệu "${data.ten}" không?`)) {
                callApiDeleteNH(data.id);
            }
            return;
        }

        // Xử lý nút EDIT
        if (e.target.closest('.btn-edit-khoi')) {
            e.stopPropagation();
            openForm('edit', data);
            return;
        }

        // Click vào dòng để XEM
        openForm('view', data);
    });

    // --- API CALLS ---

    // Lấy ID tiếp theo
    async function callApiGetNextIdNH() {
        const dataToSend = {
            prefix: "NH",
            totalLength: 5
        };
        try {
            const response = await fetch('/API/get-next-id-NH', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(dataToSend)
            });
            const data = await response.json();
            if (data && data.nextId) {
                inputId.value = data.nextId;
            } else {
                alert('Không thể lấy mã nhãn hiệu, vui lòng thử lại.');
            }
        } catch (error) {
            console.error('Lỗi khi lấy mã nhãn hiệu:', error);
            alert('Không thể lấy mã nhãn hiệu, vui lòng thử lại.');
        }
    }

    // Thêm nhãn hiệu
    async function callApiAddNH() {
        try {
            const duLieu = {
                Id: inputId.value,
                Ten: inputTen.value
            };

            // Validate
            if (!duLieu.Ten) {
                alert('Vui lòng nhập tên nhãn hiệu!');
                return;
            }

            const response = await fetch('/API/add-NH', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(duLieu)
            });

            const result = await response.json();

            if (response.ok) {
                alert('Thêm thành công!');
                closeForm();
                await reloadTable();
            } else {
                alert('Lỗi: ' + result.message);
            }
        } catch (error) {
            console.error('Lỗi:', error);
            alert('Lỗi khi thêm nhãn hiệu: ' + error.message);
        }
    }

    // Sửa nhãn hiệu
    async function callApiEditNH() {
        try {
            const duLieu = {
                Id: inputId.value,
                Ten: inputTen.value,
                IsDelete: false
            };

            // Validate
            if (!duLieu.Ten) {
                alert('Vui lòng nhập tên nhãn hiệu!');
                return;
            }

            const response = await fetch('/API/edit-NH', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(duLieu)
            });

            const result = await response.json();

            if (response.ok) {
                alert('Sửa thành công!');
                closeForm();
                await reloadTable();
            } else {
                alert('Lỗi: ' + result.message);
            }
        } catch (error) {
            console.error('Lỗi:', error);
            alert('Lỗi khi sửa nhãn hiệu: ' + error.message);
        }
    }

    // Xóa nhãn hiệu
    async function callApiDeleteNH(id) {
        try {
            const response = await fetch(`/API/delete-NH/${encodeURIComponent(id)}`, {
                method: 'DELETE'
            });

            const result = await response.json();

            if (response.ok) {
                alert('Xóa thành công!');
                closeForm();
                await reloadTable();
            } else {
                alert('Lỗi: ' + result.message);
            }
        } catch (error) {
            console.error('Lỗi:', error);
            alert('Lỗi khi xóa: ' + error.message);
        }
    }

    // Reload table thủ công
    async function reloadTable() {
        try {
            console.log('Reloading table...');
            const response = await fetch('/API/get-all-NH');
            const data = await response.json();

            const tbody = document.querySelector('#sampleTable tbody');
            tbody.innerHTML = '';

            data.forEach(nh => {
                const row = document.createElement('tr');
                row.dataset.id = nh.id;
                row.dataset.ten = nh.ten;

                row.innerHTML = `
                    <td>${nh.id}</td>
                    <td>${nh.ten}</td>
                    <td>${nh.soSanPham || 0}</td>
                    <td class="text-center">
                        <a class="btn btn-info btn-sm me-1 btn-edit-khoi" 
                           href="#" 
                           data-id="${nh.id}"
                           title="Sửa">
                            <i class="fas fa-edit"></i>
                        </a>
                        <a class="btn btn-danger btn-sm btn-delete-khoi" 
                           href="#" 
                           data-id="${nh.id}" 
                           title="Xóa">
                            <i class="fas fa-trash-alt"></i>
                        </a>
                    </td>`;

                tbody.appendChild(row);
            });

            console.log('✅ Table reloaded successfully');
        } catch (error) {
            console.error('❌ Error reloading table:', error);
        }
    }

    console.log('✅ Page initialized');
});

// --- SIGNALR REALTIME ---
$(async function () {
    console.log('=== INITIALIZING SIGNALR FOR NHAN HIEU ===');

    await appRealtimeList.initEntityTable({
        key: 'NhanHieu',
        apiUrl: '/API/get-all-NH',
        tableId: 'sampleTable',
        tbodyId: 'sampleTable tbody',
        buildRow: nh => {
            return `
                <tr data-id="${nh.id}" data-ten="${nh.ten}">
                    <td>${nh.id}</td>
                    <td>${nh.ten}</td>
                    <td>${nh.soSanPham || 0}</td>
                    <td class="text-center">
                        <a class="btn btn-info btn-sm me-1 btn-edit-khoi" 
                           href="#" 
                           data-id="${nh.id}"
                           title="Sửa">
                            <i class="fas fa-edit"></i>
                        </a>
                        <a class="btn btn-danger btn-sm btn-delete-khoi" 
                           href="#" 
                           data-id="${nh.id}" 
                           title="Xóa">
                            <i class="fas fa-trash-alt"></i>
                        </a>
                    </td>
                </tr>`;
        }
    });

    console.log('✅ SignalR initialized for NhanHieu');
});



